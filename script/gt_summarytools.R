gt_summarytools <- function(data, title = "Data Summary") {
  ...
  create_sum_table <- function(df) {
    ...
    sum_table <- purrr::map_df(vars, function(var_name) {
      ...
      # --- MODIFIED SECTION: Numeric Variables ---
      if (is.numeric(x)) {
      ...
      if (is.null(var_label) || var_label == "") {
        var_label <- "No label available"
      }
      var_display <- glue::glue("<div style='font-weight: bold;'>{var_name}</div><span style='font-weight: normal; font-size: 80%;'>{var_label}</span>")
      
      tibble(
        No = NA_integer_,
        Variable = var_display, 
        Variable_Name_Simple = var_name, 
        type = type,
  ...
  # --- Plotting function remains exactly the same ---
  plot_data <- function(col, col_name, n_missing, ...) {
    col_type <- class(col)[1]
    valid_data <- col[!is.na(col)]
    if (length(valid_data) < 2 || all(!is.finite(valid_data))) {
      return(gt::html("<div><center>Insufficient valid data for plot (N < 2)</center></div>"))
    }
    col <- valid_data 
    col <- col[!is.na(col)]
    ...
    if (col_type %in% c("factor", "character", "ordered", "logical")) {
      ...
      plot_out <- freq_df %>%
        ...
        labs(y = paste(n_unique, cat_lab))
      plot_height <- ifelse(n_unique <= 11, n_unique * 8, 8)
      
    } else if (col_type %in% c("numeric", "double", "integer")) {
      df_in <- dplyr::tibble(x = col) %>% dplyr::filter(!is.na(x))
      ...
      plot_out <- ggplot(df_in, aes(x = x)) +
        ...
        theme(
          ...
          axis.line.x = element_line(color = "black"), axis.ticks.x = element_line(color = "black"),
          axis.ticks.length.x = unit(1, "mm"), plot.margin = margin(1, 1, 3, 1),
    ...
    img_plot <- readLines(out_name, warn = FALSE) %>% paste0(collapse = "") %>% gt::html()
  ...
  gt_table <- summary_table %>%
    gt() %>%
    cols_width(
      stats_values ~ pct(25), Graph ~ pct(10), Variable ~ pct(15),
      Valid ~ pct(7), Missing ~ pct(7), Variable_Name_Simple ~ px(0)
    ) %>%
    fmt_markdown(columns = c("Variable")) %>%
    text_transform(
      locations = cells_body(columns = c("Graph")),
      fn = function(x) {
        map(seq_len(nrow(summary_table)), function(i) {
          col_name_simple <- summary_table$Variable_Name_Simple[i]
          plot_data(
            col = data[[col_name_simple]], 
            col_name = col_name_simple,
    ...
    cols_label(
      No = "No", Variable = "Variable", type = "Type",
      stats_values = "Stats / Values", Freqs_Percents = "Freqs (% of Valid)",
      Graph = "Graph", Valid = "Valid", Missing = "Missing"
    ) %>%
